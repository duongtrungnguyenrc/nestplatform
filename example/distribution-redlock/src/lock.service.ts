...
import { REDLOCK_SERVICE } from "@nestplatform/distribution-redlock";
...
export class LockService {
  ...
  async dynamicLock(id: string) {
    console.log(`[${Date.now()}] Dynamic lock acquired for id ${id}, doing work for 2s...`);
    await sleep(2000);
  ...
  async manualLock() {
    return this.lockService.withLock(["manual:lock"], 5000, async () => {
      console.log(`[${Date.now()}] Manual lock acquired, doing work for 2s...`);
      await sleep(2000);
